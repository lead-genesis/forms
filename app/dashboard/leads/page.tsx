...
import { UserGroupIcon, PhoneIcon, ChevronLeftIcon, ChevronRightIcon, CheckCircleIcon, XCircleIcon, MagnifyingGlassIcon } from "@heroicons/react/24/outline";
...
export default function LeadsPage() {
    ...
    const filteredLeads = useMemo(() => {
        ...
        return leads.filter((l) =>
            [l.answers?.first_name, l.answers?.last_name, l.answers?.email, l.answers?.phone, l.form_name]
                .filter(Boolean)
    ...
    return (
        <DashboardPage>
            <motion.div variants={fadeInUp} className="flex flex-col gap-4 w-full max-w-[70%] mx-auto">
                {/* Local header */}
                <div className="flex items-center justify-between">
                    <h3 className={cn("text-lg font-semibold tracking-tight", sansFont)}>Leads</h3>
                ...
                {isLoading ? (
                    ...
                    <div className="flex flex-col gap-0">
                        <div className="w-full overflow-x-auto">
                                <table className={tableBase + " border-collapse min-w-[900px]"}>
                                    <thead className={tableHead}>
                                        <tr>
                                            <th className={tableHeadCell}>Lead</th>
                        ...
                        {/* Pagination */}
                        {totalPages > 1 && (
                            <div className="sticky bottom-0 bg-background/95 backdrop-blur-sm border-t border-border/40 flex items-center justify-between py-3">
                                <p className="text-xs text-muted-foreground">
                                    Showing <span className="font-semibold text-foreground">{(currentPage - 1) * ITEMS_PER_PAGE + 1}</span> – <span className="font-semibold text-foreground">{Math.min(currentPage * ITEMS_PER_PAGE, filteredLeads.length)}</span> of <span className="font-semibold text-foreground">{filteredLeads.length}</span>
                                </p>
                                <div className="flex items-center gap-2">
                                    <button
